...
function pageCompare() {
  ...
  return `<div class="card" style="margin-bottom:18px;">
    <div style="display:flex;flex-wrap:wrap;gap:8px;">
      ...
      <span class="badge" style="border:1px dashed var(--navy);color:var(--navy);cursor:pointer;padding:6px 12px;" onclick="nav('universities')">+ Add</span>
    </div>
  </div>
  ${list.length < 2 ? `<div class="empty">Select at least 2 universities. <button class="btn btn-outline btn-sm" onclick="nav('universities')">Browse</button></div>`
    : `<div class="card" style="overflow-x:auto;"><table><thead><tr><th>Feature</th>${list.map(u=>`<th><div class="uni-brand" style="width:40px;height:40px;margin:0 auto 6px;">${uniBrand(u)}</div>${esc(uniShortName(u.name))}</th>`).join('')}</tr></thead>
